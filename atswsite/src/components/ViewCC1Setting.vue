...
// The relative position of this file: src/components/ViewCC1Setting.vue

// 定义组件属性
interface Props {
    openSettingState: boolean
}
...
const emit = defineEmits<{
  'click-open-setting': [value: boolean]
}>()

// 方法
const handleClick = () => {
  emit('click-open-setting', props.openSettingState)
}
</script>
...
<template>
  <div class="view-setting-container">
    <a 
      class="button" 
      :class="props.openSettingState ? 'openState' : 'closeState'"
      @click="handleClick"
      role="button"
      tabindex="0"
