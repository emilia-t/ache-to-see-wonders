...
const config = {
  ...
  ShowDirectionIndicator: true // 新增：显示方向指示器
};
...
class WebGLRenderer {
  ...
  // 方向指示器相关
  private directionIndicatorVertices: Float32Array | null = null;
  private directionIndicatorColors: Float32Array | null = null;
  private directionIndicatorVertexCount = 0;
  ...
  constructor(
    ...
    // 初始化方向指示器
    this.initDirectionIndicator();
  }

  /**
   * 初始化方向指示器
   */
  private initDirectionIndicator(): void {
    ...
    vertices.push(1, centerY, centerZ);
    colors.push(1, 0, 0, 1, 1, 0, 0, 1);
    
    // Y轴 - 绿色
    vertices.push(centerX, centerY, centerZ);
    vertices.push(centerX, 1, centerZ);
    colors.push(0, 1, 0, 1, 0, 1, 0, 1);
    
    // Z轴 - 蓝色
    vertices.push(centerX, centerY, centerZ);
    vertices.push(centerX, centerY, 1);
    colors.push(0, 0, 1, 1, 0, 0, 1, 1);
    
    this.directionIndicatorVertices = new Float32Array(vertices);
    this.directionIndicatorColors = new Float32Array(colors);
    this.directionIndicatorVertexCount = vertices.length / 3;
  }
  ...
  private drawDirectionIndicator(): void {
    if (!config.ShowDirectionIndicator) return;
    
    const gl = this.gl;
    
    // 保存当前矩阵状态
    const savedModelMatrix = mat4.clone(this.modelMatrix);
    const savedViewMatrix = mat4.clone(this.viewMatrix);
    const savedProjectionMatrix = mat4.clone(this.projectionMatrix);
    
    // 设置正交投影用于UI元素
    const uiProjectionMatrix = mat4.create();
    mat4.ortho(uiProjectionMatrix, -10, 10, -10, 10, -100, 100);
    
    // 设置视图矩阵 - 将指示器固定在右下角
    const uiViewMatrix = mat4.create();
    mat4.lookAt(uiViewMatrix, 
      [0, 0, 5], // 相机位置
      [0, 0, 0], // 目标位置
      [0, 1, 0]  // 上方向
    );
    ...
    // 设置模型矩阵 - 缩放和位置
    const uiModelMatrix = mat4.create();
    mat4.translate(uiModelMatrix, uiModelMatrix, [7, -7, 0]); // 右下角位置
    mat4.scale(uiModelMatrix, uiModelMatrix, [2, 2, 2]); // 缩放
    
    // 应用UI矩阵
    gl.uniformMatrix4fv(this.uProjectionMatrixLocation, false, uiProjectionMatrix);
    gl.uniformMatrix4fv(this.uViewMatrixLocation, false, uiViewMatrix);
    gl.uniformMatrix4fv(this.uModelMatrixLocation, false, uiModelMatrix);
    
    // 上传方向指示器数据
    gl.bindBuffer(gl.ARRAY_BUFFER, this.positionBuffer!);
    gl.bufferData(gl.ARRAY_BUFFER, this.directionIndicatorVertices!, gl.STATIC_DRAW);
    
    gl.bindBuffer(gl.ARRAY_BUFFER, this.colorBuffer!);
    gl.bufferData(gl.ARRAY_BUFFER, this.directionIndicatorColors!, gl.STATIC_DRAW);
    ...
    // 绘制线条
    gl.drawArrays(gl.LINES, 0, this.directionIndicatorVertexCount);
    
    // 恢复矩阵状态
    gl.uniformMatrix4fv(this.uProjectionMatrixLocation, false, savedProjectionMatrix);
    gl.uniformMatrix4fv(this.uViewMatrixLocation, false, savedViewMatrix);
    gl.uniformMatrix4fv(this.uModelMatrixLocation, false, savedModelMatrix);
  }
  ...
  handleMouseMove(e: MouseEvent): void {
    if (this.isDragging.value) {
      ...
      // 计算基于当前视角的移动向量
      const moveX = deltaX * Math.cos(azimuthRad) - deltaY * Math.sin(azimuthRad);
      const moveY = deltaX * Math.sin(azimuthRad) + deltaY * Math.cos(azimuthRad);
      
      this.cameraX.value += moveX * moveScale;
      this.cameraY.value -= moveY * moveScale;
  ...
  handleWheel(e: WheelEvent): void {
    ...
    const worldX = this.cameraX.value + (ndcX * aspect * tanFov * 20) / oldZoom;
    const worldY = this.cameraY.value + (ndcY * tanFov * 20) / oldZoom;
    ...
    // 计算新的世界坐标
    const newWorldX = this.cameraX.value + (ndcX * aspect * tanFov * 20) / this.zoomLevel.value;
    const newWorldY = this.cameraY.value + (ndcY * tanFov * 20) / this.zoomLevel.value;
    
    // 调整相机位置以保持鼠标指向的点不变
    this.cameraX.value += worldX - newWorldX;
    this.cameraY.value += worldY - newWorldY;
...
<template>
  <div class="page-neuro-sandbox-container">
    ...
    <!-- 方向指示器说明 -->
    <div v-if="config.ShowDirectionIndicator" class="direction-indicator-info">
      <div class="axis-label x-axis">X轴 (红)</div>
      <div class="axis-label y-axis">Y轴 (绿)</div>
      <div class="axis-label z-axis">Z轴 (蓝)</div>
    </div>
  </div>
